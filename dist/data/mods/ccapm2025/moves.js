...
const Moves = {
  ...
  acupressure: {
    inherit: true,
    onHit(target) {
      if (target.species.name === "Regigigas" && !this.ruleTable.has("+tag:cap")) {
        if (!target.m.stats) {
          const allStats = [];
          let stat;
  ...
  aromatherapy: {
    ...
    onHit(target, source, move) {
      ...
      for (const ally of allies) {
        ...
        if (ally.cureStatus(false, source)) success = true;
      }
      return success;
    }
  },
  ...
  explosion: {
    ...
    onAfterMove(pokemon, source, move) {
      ...
      if (pokemon.species.name === "Electrode" && pokemon.fainted) {
        this.add("-message", `Shakite escaped from its Pok\xE9ball!`);
        const shakite = {
          name: "Shakite",
          species: "Shakite",
  ...
  junglehealing: {
    ...
    onHit(pokemon) {
      ...
      return pokemon.cureStatus(false, pokemon) || success;
    }
  },
  healingwish: {
    inherit: true,
  ...
  holdhands: {
    inherit: true,
    isNonstandard: void 0,
    onAfterMove(source, target, move) {
      if (this.ruleTable.has("+tag:cap")) return;
      if (source.baseSpecies.name === "Jirachi") return;
      if (!source.side.holdHandsUsers) source.side.holdHandsUsers = [source.baseSpecies.name];
      else if (!source.side.holdHandsUsers.includes(source.baseSpecies.name)) source.side.holdHandsUsers.push(source.baseSpecies.name);
    },
  ...
  psychoshift: {
    ...
    self: {
      onHit(pokemon) {
        pokemon.cureStatus(false, pokemon);
      }
    }
  },
  purify: {
    inherit: true,
    onHit(target, source) {
      if (!target.cureStatus(false, source)) {
  ...
  smellingsalts: {
    inherit: true,
    onHit(target, source, move) {
      if (target.status === "par") target.cureStatus(false, source);
    }
  },
  sparklingaria: {
    inherit: true,
    onAfterMove(source, target, move) {
      ...
      for (const pokemon of move.hitTargets) {
        if (pokemon !== source && pokemon.isActive && (pokemon.removeVolatile("sparklingaria") || numberTargets > 1) && pokemon.status === "brn") {
          pokemon.cureStatus(false, source);
        }
      }
    }
  },
  ...
  toxicspikes: {
    ...
    condition: {
      ...
      onSwitchIn(pokemon) {
        ...
        } else if (pokemon.hasType("Steel") || pokemon.hasItem("heavydutyboots") || pokemon.hasItem("restorationcapsule") || pokemon.hasAbility("moltencore")) {
  ...
  uproar: {
    ...
    onTryHit(target, source, move) {
      ...
      for (const [i, allyActive] of activeTeam.entries()) {
        if (allyActive && allyActive.status === "slp") allyActive.cureStatus(false, source);
        const foeActive = foeActiveTeam[i];
        if (foeActive && foeActive.status === "slp") foeActive.cureStatus(false, source);
      }
    },
    secondary: void 0,
    target: "randomNormal",
  ...
  wakeupslap: {
    inherit: true,
    onHit(target, source, move) {
      if (target.status === "slp") target.cureStatus(false, source);
    }
  },
  worryseed: {
    inherit: true,
  ...
  sixtongueemojis: {
    ...
    beforeTurnCallback(pokemon) {
      pokemon.addVolatile(
        "sixtongueemojis",
        pokemon,
        this.dex.getActiveMove("sixtongueemojis")
      );
    },
    ...
    condition: {
      ...
      onFoeBeforeSwitchOut(pokemon) {
        ...
        if (!source.isAdjacent(pokemon) || !source.hp || source.volatiles["encore"] && source.volatiles["encore"].move !== "sixtongueemojis" || !this.queue.cancelMove(source)) return;
  ...
  glacierfang: {
    // placeholder but the move change would probably be coded elsewhere
    accuracy: 100,
    basePower: 85,
    category: "Physical",
    name: "Glacier Fang",
    ...
    condition: {
      ...
      onTryHit(target, source, move) {
        ...
        if (this.checkMoveMakesContact(move, source, target)) {
          const boosts = this.ruleTable.has("+tag:cap") ? { atk: -1 } : { atk: -1, spa: -1 };
  ...
  razorshell: {
    ...
    onModifyMove(move, pokemon, target) {
      if (!target || this.ruleTable.has("+tag:cap")) return;
  ...
  plasmafists: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move) {
      if (!this.effectState.bigZera && !this.ruleTable.has("+tag:cap") && pokemon.baseSpecies.baseSpecies === "Zeraora" && pokemon.volatiles["charge"] && !pokemon.transformed) {
  ...
  morningsun: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move) {
      ...
      if (pokemon.species.name === "Volcarona" && !this.ruleTable.has("+tag:cap")) {
  ...
  chillyreception: {
    ...
    condition: {
      ...
      onBeforeMove(source, target, move) {
        ...
        if (source.species.name === "Slowking" && !this.ruleTable.has("+tag:cap")) {
  ...
  /* synthesis: {
  	inherit: true,
  	beforeMoveCallback(pokemon) {
  		if (pokemon.species.name === 'Shaymin' && !this.ruleTable.has("+tag:cap")) {
  			pokemon.formeChange('Shaymin-Sky', null, true);
  			pokemon.setAbility('serenegrace', pokemon);
  			return true;
  		}
  ...
  seedflare: {
  	inherit: true,
  	beforeMoveCallback(pokemon) {
  		if (pokemon.species.name === 'Shaymin-Sky' && !this.ruleTable.has("+tag:cap")) {
  ...
  technoblast: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move) {
      ...
      if (pokemon.species.name === "Genesect-Burn" && !this.ruleTable.has("+tag:cap")) {
        pokemon.formeChange("Genesect-Core", null, true);
        pokemon.setAbility("martialize", pokemon);
        this.effectState.geneSect = true;
      } else if (pokemon.species.name === "Genesect-Douse" && !this.ruleTable.has("+tag:cap")) {
        pokemon.formeChange("Genesect-Rust", null, true);
        pokemon.setAbility("intoxicate", pokemon);
        this.effectState.geneSect = true;
      } else if (pokemon.species.name === "Genesect-Shock" && !this.ruleTable.has("+tag:cap")) {
        pokemon.formeChange("Genesect-Airborne", null, true);
        pokemon.setAbility("aerilate", pokemon);
        this.effectState.geneSect = true;
      } else if (pokemon.species.name === "Genesect-Chill" && !this.ruleTable.has("+tag:cap")) {
  ...
  shellsmash: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move) {
      ...
      if (pokemon.species.name === "Crustle" && !this.ruleTable.has("+tag:cap")) {
  ...
  curse: {
    ...
    onAfterMoveSecondarySelf(pokemon, target) {
      if (pokemon.species.name === "Cofagrigus" && !this.ruleTable.has("+tag:cap")) {
        pokemon.formeChange("Cofagrigus-Unchained", null, true);
        pokemon.setAbility("darkmagic", pokemon);
        const curseIndex = pokemon.set.moves.map((move2) => move2.toLowerCase().replace(/[^a-z0-9]/g, "")).indexOf("curse");
  ...
  phantomforce: {
    ...
    onTryMove(attacker, defender, move) {
      ...
      if (attacker.species.name === "Dragapult" && !attacker.transformed && !this.ruleTable.has("+tag:cap")) {
  ...
  moneytrees: {
    ...
    onBasePower(basePower, source) {
      if (this.field.isTerrain("grassyterrain") && source.isGrounded() || ["sunnyday", "desolateland"].includes(source.effectiveWeather())) {
