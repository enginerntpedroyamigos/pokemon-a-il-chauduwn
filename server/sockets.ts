...
export class ServerStream extends Streams.ObjectReadWriteStream<string> {
	...
	constructor(config: {
		...
		try {
			...
			// MOD AÑIL: nuestro cliente propio (con Royaleon/Cefireon) vive acá y se
			// sirve en el mismo puerto/origen que el servidor de batalla — así no
			// hace falta un segundo servicio ni trucos de origen cruzado.
			const clientServer = new StaticServer('./server/static-client');
			const proxyToOfficialClient = (req: http.IncomingMessage, res: http.ServerResponse) => {
				const upstreamReq = https.request({
					host: 'play.pokemonshowdown.com',
					path: req.url,
					method: req.method,
					headers: { ...req.headers, host: 'play.pokemonshowdown.com' },
				}, upstreamRes => {
				...
				req.addListener('end', () => {
					...
					if (req.url) {
						if (req.url === '/custom.css' || req.url.startsWith('/custom.css?')) {
							...
							server = avatarServer;
						} else if (req.url === '/robots.txt' || req.url === '/favicon.ico') {
							server = staticServer;
					...
					void server.serve(req, res, e => {
						if (e.status === 404) {
							if (server === clientServer) {
								// No lo tenemos en nuestro cliente local -> lo pedimos
								// prestado en vivo al Showdown oficial (sprites, sonidos,
								// etc. que no tocamos).
								proxyToOfficialClient(req, res);
								return true;
							}
