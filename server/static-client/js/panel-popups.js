...
StatusEditor=function(_preact$Component){function StatusEditor(){var _this2;for(var _len=arguments.length,args=new Array(_len),_key=0;_key<_len;_key++){args[_key]=arguments[_key];}_this2=_preact$Component.call.apply(_preact$Component,[this].concat(args))||this;_this2.
...
addStatus=function(ev){
ev.preventDefault();
ev.stopImmediatePropagation();
_this2.setState({addingStatus:true},function(){return _this2.focusStatus();});
};_this2.
focusStatus=function(ev){
ev==null||ev.preventDefault();
ev==null||ev.stopImmediatePropagation();
var input=_this2.statusInput();
input==null||input.focus();
input==null||input.select();
};_this2.
handleStatusInput=function(ev){
var changed=ev.currentTarget.value!==_this2.currentStatus();
if(changed!==!!_this2.state.statusChanged)_this2.setState({statusChanged:changed});
};_this2.
handleStatusKeyDown=function(ev){
if(ev.key==='Escape'){
ev.preventDefault();
ev.stopImmediatePropagation();
_this2.revertStatus();
...
_this2.setState({addingStatus:false,statusChanged:false});
};_this2.
submitStatus=function(ev){var _this2$statusInput;
ev.preventDefault();
ev.stopImmediatePropagation();
var status=(((_this2$statusInput=_this2.statusInput())==null?void 0:_this2$statusInput.value)||'').trim();
PS.send(status?"/noreply /status "+status:"/noreply /clearstatus");

var details=PS.mainmenu.userdetailsCache[PS.user.userid];
if(details){
var awayMarkers=/^!?(?:\((?:Busy|Idle)\) ?)?/.exec(details.status||'')[0];
details.status=awayMarkers+status;
}
_this2.setState({addingStatus:false,statusChanged:false});
_this2.fetchStatus();
};return _this2;}_inheritsLoose(StatusEditor,_preact$Component);var _proto2=StatusEditor.prototype;_proto2.fetchStatus=function fetchStatus(){var _this3=this;if(!PS.user.userid)return;PS.mainmenu.makeQuery('userdetails',PS.user.userid).then(function(){return _this3.forceUpdate();});};_proto2.currentStatus=function currentStatus(){var _PS$mainmenu$userdeta;var status=((_PS$mainmenu$userdeta=PS.mainmenu.userdetailsCache[PS.user.userid])==null?void 0:_PS$mainmenu$userdeta.status)||'';if(status.startsWith('!'))status=status.slice(1);return status.replace(/^\((?:Busy|Idle)\) ?/,'');};_proto2.statusLabel=function statusLabel(){var _PS$mainmenu$userdeta2,_exec;var status=((_PS$mainmenu$userdeta2=PS.mainmenu.userdetailsCache[PS.user.userid])==null?void 0:_PS$mainmenu$userdeta2.status)||'';return((_exec=/^!?(\((?:Busy|Idle)\))/.exec(status))==null?void 0:_exec[1])||'';};_proto2.statusInput=function statusInput(){return this.base.querySelector('textarea[name=statustext]');};_proto2.
render=function render(){
var curStatus=this.currentStatus();
var label=this.statusLabel();
if(!curStatus&&!this.state.addingStatus){
return preact.h("div",null,
label&&preact.h("span",{"class":"userstatus",style:"display:inline"},label)," ",
preact.h("button",{"class":"button small",onClick:this.addStatus},"Add status")
);
}
return preact.h("form",{onSubmit:this.submitStatus,"class":"statusform"},
label&&preact.h("span",{"class":"userstatus",style:"display:inline"},label)," ",
preact.h(PSTextarea,{
"class":"userstatus",
key:curStatus,singleLine:true,subtle:true,name:"statustext",defaultValue:curStatus,
...
preact.h("button",{"class":"button",type:"submit"},
preact.h("strong",null,curStatus?'Change':'Add')
)," ",
preact.h("button",{"class":"button",type:"button",onClick:this.revertStatus},
curStatus?'Revert':'Cancel'
)
):

preact.h("button",{"class":"button small",type:"button",onClick:this.focusStatus,"aria-label":"Edit"},
preact.h("i",{"class":"fa fa-pencil","aria-hidden":true})
)

);
};return StatusEditor;}(preact.Component);var


UserPanel=function(_PSRoomPanel){function UserPanel(){var _this4;for(var _len2=arguments.length,args=new Array(_len2),_key2=0;_key2<_len2;_key2++){args[_key2]=arguments[_key2];}_this4=_PSRoomPanel.call.apply(_PSRoomPanel,[this].concat(args))||this;_this4.
...
};return UserPanel;}(PSRoomPanel);UserPanel.id='user';UserPanel.routes=['user-*','viewuser-*','users'];UserPanel.Model=UserRoom;UserPanel.location='popup';


var USER_PUNISHMENTS={
...
UserOptionsPanel=function(_PSRoomPanel2){function UserOptionsPanel(){var _this5;for(var _len3=arguments.length,args=new Array(_len3),_key3=0;_key3<_len3;_key3++){args[_key3]=arguments[_key3];}_this5=_PSRoomPanel2.call.apply(_PSRoomPanel2,[this].concat(args))||this;_this5.
...
handleCommand=function(ev){
ev.preventDefault();
ev.stopImmediatePropagation();
var command=ev.currentTarget.getAttribute('data-cmdpreview');
if(!command)return PS.alert("Room not found",{parentElem:ev.currentTarget});
_this5.send(command);
};_this5.
...
openPunishment=function(ev){
var button=ev.currentTarget;
var punishment=button.value;
ev.preventDefault();
ev.stopImmediatePropagation();
if(!(punishment in USER_PUNISHMENTS))return;

var _this5$getTargets=_this5.getTargets(),targetUser=_this5$getTargets.targetUser,targetRoom=_this5$getTargets.targetRoom;
var action=USER_PUNISHMENTS[punishment];
var scope=action.scope==='room'?" in "+((targetRoom==null?void 0:targetRoom.title)||'???'):'';
PS.prompt(
...
reason=reason.trim();
_this5.send("/"+punishment+" "+targetUser+(reason?", "+reason:''));
});
};_this5.
...
return false;
};return _this5;}_inheritsLoose(UserOptionsPanel,_PSRoomPanel2);var _proto4=UserOptionsPanel.prototype;_proto4.getTargets=function getTargets(){var _targetRoom,_targetRoom2,_targetRoom3,_targetRoom4,_targetRoom5,_targetRoom6;var _PSUtils$splitFirst=PSUtils.splitFirst(this.props.room.id,'-',2),targetUser=_PSUtils$splitFirst[1],targetRoomid=_PSUtils$splitFirst[2];var targetRoom=(targetRoomid?PS.rooms[targetRoomid]:null)||null;if(((_targetRoom=targetRoom)==null?void 0:_targetRoom.type)!=='chat')targetRoom=(_targetRoom2=targetRoom)==null?void 0:_targetRoom2.getParent();if(((_targetRoom3=targetRoom)==null?void 0:_targetRoom3.type)!=='chat')targetRoom=(_targetRoom4=targetRoom)==null?void 0:_targetRoom4.getParent();if(((_targetRoom5=targetRoom)==null?void 0:_targetRoom5.type)!=='chat')targetRoom=null;if((_targetRoom6=targetRoom)!=null&&_targetRoom6.pmTarget)targetRoom=null;return{targetUser:targetUser,targetRoomid:targetRoomid,targetRoom:targetRoom};};_proto4.send=function send(command){var _this$getTargets=this.getTargets(),targetUser=_this$getTargets.targetUser,targetRoom=_this$getTargets.targetRoom;if(targetRoom){targetRoom.send(command);}else{PS.join("dm-"+targetUser);PS.rooms["dm-"+targetUser].send(command);}this.close();};_proto4.

render=function render(){var _targetRoom$users$PS$;
var room=this.props.room;
var _this$getTargets2=this.getTargets(),targetUser=_this$getTargets2.targetUser,targetRoom=_this$getTargets2.targetRoom;
var userRoomGroup=targetRoom==null||(_targetRoom$users$PS$=targetRoom.users[PS.user.userid])==null?void 0:_targetRoom$users$PS$.charAt(0);
var roomGroupOrder=PS.server.getGroup(userRoomGroup).order;
var globalGroupOrder=PS.server.getGroup(PS.user.group).order;
var canMute=!!targetRoom&&roomGroupOrder<=PS.server.getGroup('%').order;
var canBan=!!targetRoom&&roomGroupOrder<=PS.server.getGroup('@').order;
var canLock=globalGroupOrder<=PS.server.getGroup('%').order;

return preact.h(PSPanelWrapper,{room:room,width:280},preact.h("div",{"class":"pad buttonmenu"},
preact.h("p",null,
preact.h("button",{"data-cmd":"/dm "+targetUser+", /friend add "+targetUser,"class":"button"},"Add friend"

)
),
preact.h("p",null,
this.isIgnoringUser(targetUser)?
preact.h("button",{"data-cmdpreview":"/unignore "+targetUser,onClick:this.handleCommand,"class":"button"},"Unignore"

):

preact.h("button",{"data-cmdpreview":"/ignore "+targetUser,onClick:this.handleCommand,"class":"button"},"Ignore"
...
(canMute||canBan)&&preact.h("fieldset",{"class":"fieldset"},
preact.h("legend",null,"Moderate in ",targetRoom==null?void 0:targetRoom.title),
canMute&&preact.h("p",null,
preact.h("button",{
"class":"button button-first",value:"mute",onClick:this.openPunishment,
"data-cmdpreview":"/mute "+targetUser},
"Mute ",
preact.h("small",null,"7m")
),
preact.h("button",{
"class":"button button-last",value:"hourmute",onClick:this.openPunishment,
"data-cmdpreview":"/hourmute "+targetUser},
"Hourmute ",
preact.h("small",null,"1h")
)
),
canBan&&preact.h("p",null,
preact.h("button",{
"class":"button button-first",value:"ban",onClick:this.openPunishment,
"data-cmdpreview":"/ban "+targetUser},
"Ban ",
preact.h("small",null,"2d")
),
preact.h("button",{
"class":"button button-last",value:"weekban",onClick:this.openPunishment,
"data-cmdpreview":"/weekban "+targetUser},
"Weekban ",
preact.h("small",null,"1w")
)
),
preact.h("p",null,
preact.h("button",{
"data-cmdpreview":"/modlog userid="+targetUser+", room="+(targetRoom==null?void 0:targetRoom.id),onClick:this.handleCommand,"class":"button"},
"Modlog"

)
)
),
canLock&&preact.h("fieldset",{"class":"fieldset"},
preact.h("legend",null,"Global moderation"),
preact.h("p",null,
preact.h("button",{
...
UserListPanel=function(_PSRoomPanel3){function UserListPanel(){return _PSRoomPanel3.apply(this,arguments)||this;}_inheritsLoose(UserListPanel,_PSRoomPanel3);var _proto5=UserListPanel.prototype;_proto5.
...
};return UserListPanel;}(PSRoomPanel);UserListPanel.id='userlist';UserListPanel.routes=['userlist'];UserListPanel.location='modal-popup';UserListPanel.noURL=true;var


VolumePanel=function(_PSRoomPanel4){function VolumePanel(){var _this6;for(var _len4=arguments.length,args=new Array(_len4),_key4=0;_key4<_len4;_key4++){args[_key4]=arguments[_key4];}_this6=_PSRoomPanel4.call.apply(_PSRoomPanel4,[this].concat(args))||this;_this6.
...
PS.update();
};return _this6;}_inheritsLoose(VolumePanel,_PSRoomPanel4);var _proto6=VolumePanel.prototype;_proto6.
componentDidMount=function componentDidMount(){var _this7=this;
...
OptionsPanel=function(_PSRoomPanel5){function OptionsPanel(){var _this8;for(var _len5=arguments.length,args=new Array(_len5),_key5=0;_key5<_len5;_key5++){args[_key5]=arguments[_key5];}_this8=_PSRoomPanel5.call.apply(_PSRoomPanel5,[this].concat(args))||this;_this8.
...
switch(setting){
case'blockPMs':{
PS.prefs.set('serversettings',Object.assign({},PS.prefs.serversettings,{blockPMs:value}));
PS.send(value?'/blockpms':'/unblockpms');
break;
}
case'blockChallenges':{
PS.prefs.set('serversettings',Object.assign({},PS.prefs.serversettings,{blockChallenges:value}));
...
case'language':{
PS.prefs.set('serversettings',Object.assign({},PS.prefs.serversettings,{language:elem.value}));
void Dex.loadTextData().then(function(){
for(var roomid in PS.rooms){var _PS$rooms$roomid;
var battle=(_PS$rooms$roomid=PS.rooms[roomid])==null?void 0:_PS$rooms$roomid.battle;
...
};return _this8;}_inheritsLoose(OptionsPanel,_PSRoomPanel5);var _proto7=OptionsPanel.prototype;_proto7.componentDidMount=function componentDidMount(){var _this9=this;_PSRoomPanel5.prototype.componentDidMount.call(this);this.subscribeTo(PS.user);PS.mainmenu.makeQuery('userdetails',PS.user.userid).then(function(){return _this9.forceUpdate();});};_proto7.

render=function render(){var _PS$user$registered;
var room=this.props.room;
var serverSettings=PS.prefs.serversettings;
return preact.h(PSPanelWrapper,{room:room,width:340},preact.h("div",{"class":"pad"},
preact.h("p",{style:"padding-left:50px"},
preact.h("img",{
...
preact.h(StatusEditor,null)
),

preact.h("p",{style:"clear:both"},
...
preact.h("hr",null),
preact.h("h3",null,"General"),
preact.h("p",null,
preact.h("label",{"class":"optlabel"},
preact.h("i",{"class":"fa fa-globe","aria-hidden":true})," Language: ",
preact.h("select",{name:"language",onChange:this.handleOnChange,"class":"select",value:serverSettings.language||'english'},
preact.h("option",{value:"english"},"English"),
preact.h("option",{value:"german"},"Deutsch"),
preact.h("option",{value:"spanish"},"Espa\xF1ol"),
preact.h("option",{value:"french"},"Fran\xE7ais"),
preact.h("option",{value:"italian"},"Italiano"),
preact.h("option",{value:"dutch"},"Nederlands"),
preact.h("option",{value:"portuguese"},"Portugu\xEAs"),
preact.h("option",{value:"turkish"},"T\xFCrk\xE7e"),
preact.h("option",{value:"hindi"},"\u0939\u093F\u0902\u0926\u0940"),
preact.h("option",{value:"japanese"},"\u65E5\u672C\u8A9E"),
preact.h("option",{value:"korean"},"\uD55C\uAD6D\uC5B4"),
preact.h("option",{value:"simplifiedchinese"},"\u7B80\u4F53\u4E2D\u6587"),
preact.h("option",{value:"traditionalchinese"},"\u7E41\u9AD4\u4E2D\u6587")
)
)
),
preact.h("hr",null),
preact.h("h3",null,"Appearance"),
preact.h("p",null,
preact.h("label",{"class":"optlabel"},"Theme: ",preact.h("select",{
...
preact.h("option",{value:"system"},"Match system theme")
))
),
preact.h("p",null,
preact.h("label",{"class":"optlabel"},"Layout: ",preact.h("select",{
name:"layout","class":"select",onChange:this.setLayout,
value:PS.prefs.onepanel===true?'onepanel':PS.prefs.onepanel||''},

preact.h("option",{value:""},
...
preact.h("option",{value:"onepanel"},"Single panel"),
preact.h("option",{value:"vertical"},"Vertical tabs")
))
),
preact.h("p",null,
...
preact.h("option",{value:"notify"},"Always notify"),
preact.h("option",{value:"nonotify"},"Notify when joined"),
preact.h("option",{value:"hide"},"Hide")
)
)
),
preact.h("p",null,
preact.h("label",{"class":"optlabel"},"Timestamps: ",preact.h("select",{
name:"layout","class":"select",onChange:this.setChatroomTimestamp,value:PS.prefs.timestamps.chatrooms||''},

preact.h("option",{value:""},"Off"),
preact.h("option",{value:"minutes"},"[HH:MM]"),
preact.h("option",{value:"seconds"},"[HH:MM:SS]")
))
),
preact.h("p",null,
preact.h("label",{"class":"optlabel"},"Timestamps in DMs: ",preact.h("select",{
...
};return OptionsPanel;}(PSRoomPanel);OptionsPanel.id='options';OptionsPanel.routes=['options'];OptionsPanel.location='modal-popup';var


GooglePasswordBox=function(_preact$Component2){function GooglePasswordBox(){return _preact$Component2.apply(this,arguments)||this;}_inheritsLoose(GooglePasswordBox,_preact$Component2);var _proto8=GooglePasswordBox.prototype;_proto8.
componentDidMount=function componentDidMount(){var _this10=this;
window.gapiCallback=function(response){
PS.user.changeNameWithPassword(_this10.props.name,response.credential,{needsGoogle:true});
};
...
LoginPanel=function(_PSRoomPanel6){function LoginPanel(){var _this11;for(var _len6=arguments.length,args=new Array(_len6),_key6=0;_key6<_len6;_key6++){args[_key6]=arguments[_key6];}_this11=_PSRoomPanel6.call.apply(_PSRoomPanel6,[this].concat(args))||this;_this11.
...
ev.preventDefault();
var passwordBox=_this11.base.querySelector('input[name=password]');
if(passwordBox){
PS.user.changeNameWithPassword(_this11.getUsername(),passwordBox.value);
}else{
PS.user.changeName(_this11.getUsername());
}
};_this11.
update=function(){
_this11.forceUpdate();
...
ev.preventDefault();
ev.stopImmediatePropagation();
_this11.props.room.args=null;
_this11.forceUpdate();
};_this11.
handleShowPassword=function(ev){
ev.preventDefault();
ev.stopImmediatePropagation();
_this11.setState({passwordShown:!_this11.state.passwordShown});
};return _this11;}_inheritsLoose(LoginPanel,_PSRoomPanel6);var _proto9=LoginPanel.prototype;_proto9.componentDidMount=function componentDidMount(){var _this12=this;_PSRoomPanel6.prototype.componentDidMount.call(this);this.subscriptions.push(PS.user.subscribe(function(args){if(args){if(args.success){_this12.close();return;}_this12.props.room.args=args;setTimeout(function(){return _this12.focus();},1);}_this12.forceUpdate();}));};_proto9.getUsername=function getUsername(){var _this$props$room$args,_this$base;var loginName=PS.user.loggingIn||((_this$props$room$args=this.props.room.args)==null?void 0:_this$props$room$args.name);if(loginName)return loginName;var input=(_this$base=this.base)==null?void 0:_this$base.querySelector('input[name=username]');if(input&&!input.disabled){return input.value;}return PS.user.named?PS.user.name:'';};_proto9.focus=function focus(_options){var _ref;var passwordBox=this.base.querySelector('input[name=password]');var usernameBox=this.base.querySelector('input[name=username]');(_ref=passwordBox||usernameBox)==null||_ref.select();};_proto9.
render=function render(){
var room=this.props.room;
...
};return LoginPanel;}(PSRoomPanel);LoginPanel.id='login';LoginPanel.routes=['login'];LoginPanel.location='modal-popup';var


AvatarsPanel=function(_PSRoomPanel7){function AvatarsPanel(){return _PSRoomPanel7.apply(this,arguments)||this;}_inheritsLoose(AvatarsPanel,_PSRoomPanel7);var _proto10=AvatarsPanel.prototype;_proto10.
...
};return AvatarsPanel;}(PSRoomPanel);AvatarsPanel.id='avatars';AvatarsPanel.routes=['avatars'];AvatarsPanel.location='modal-popup';var


BattleForfeitPanel=function(_PSRoomPanel8){function BattleForfeitPanel(){return _PSRoomPanel8.apply(this,arguments)||this;}_inheritsLoose(BattleForfeitPanel,_PSRoomPanel8);var _proto11=BattleForfeitPanel.prototype;_proto11.
...
battleRoom.battle&&!battleRoom.battle.rated&&preact.h("button",{type:"button","data-href":"replaceplayer","class":"button"},"Replace player"
...
};return BattleForfeitPanel;}(PSRoomPanel);BattleForfeitPanel.id='forfeit';BattleForfeitPanel.routes=['forfeitbattle'];BattleForfeitPanel.location='modal-popup';BattleForfeitPanel.noURL=true;var


ReplacePlayerPanel=function(_PSRoomPanel9){function ReplacePlayerPanel(){var _this13;for(var _len7=arguments.length,args=new Array(_len7),_key7=0;_key7<_len7;_key7++){args[_key7]=arguments[_key7];}_this13=_PSRoomPanel9.call.apply(_PSRoomPanel9,[this].concat(args))||this;_this13.





handleReplacePlayer=function(ev){var _room$getParent,_this13$base;
var room=_this13.props.room;
var battleRoom=(_room$getParent=room.getParent())==null?void 0:_room$getParent.getParent();
var newPlayer=(_this13$base=_this13.base)==null||(_this13$base=_this13$base.querySelector("input[name=newplayer]"))==null?void 0:_this13$base.value;
if(!(newPlayer!=null&&newPlayer.length))return battleRoom.errorReply("Enter player's name");
...
_this13.close();
ev.preventDefault();
};return _this13;}_inheritsLoose(ReplacePlayerPanel,_PSRoomPanel9);var _proto12=ReplacePlayerPanel.prototype;_proto12.
...
};return ReplacePlayerPanel;}(PSRoomPanel);ReplacePlayerPanel.id='replaceplayer';ReplacePlayerPanel.routes=['replaceplayer'];ReplacePlayerPanel.location='modal-popup';ReplacePlayerPanel.noURL=true;var


ChangePasswordPanel=function(_PSRoomPanel10){function ChangePasswordPanel(){var _this14;for(var _len8=arguments.length,args=new Array(_len8),_key8=0;_key8<_len8;_key8++){args[_key8]=arguments[_key8];}_this14=_PSRoomPanel10.call.apply(_PSRoomPanel10,[this].concat(args))||this;_this14.
...
handleChangePassword=function(ev){var _this14$base,_this14$base2,_this14$base3;
ev.preventDefault();
var oldpassword=(_this14$base=_this14.base)==null||(_this14$base=_this14$base.querySelector('input[name=oldpassword]'))==null?void 0:_this14$base.value;
var password=(_this14$base2=_this14.base)==null||(_this14$base2=_this14$base2.querySelector('input[name=password]'))==null?void 0:_this14$base2.value;
var cpassword=(_this14$base3=_this14.base)==null||(_this14$base3=_this14$base3.querySelector('input[name=cpassword]'))==null?void 0:_this14$base3.value;
if(!(oldpassword!=null&&oldpassword.length)||
!(password!=null&&password.length)||
!(cpassword!=null&&cpassword.length))return _this14.setState({errorMsg:"All fields are required"});
if(password!==cpassword)return _this14.setState({errorMsg:'Passwords do not match'});
PSLoginServer.query("changepassword",{
oldpassword:oldpassword,
password:password,
cpassword:cpassword
}).then(function(data){
if(data!=null&&data.actionerror)return _this14.setState({errorMsg:data==null?void 0:data.actionerror});
PS.alert("Your password was successfully changed!");

})["catch"](function(err){
console.error(err);
_this14.setState({errorMsg:err.message});
});

_this14.setState({errorMsg:''});
};return _this14;}_inheritsLoose(ChangePasswordPanel,_PSRoomPanel10);var _proto13=ChangePasswordPanel.prototype;_proto13.
...
};return ChangePasswordPanel;}(PSRoomPanel);ChangePasswordPanel.id="changepassword";ChangePasswordPanel.routes=["changepassword"];ChangePasswordPanel.location="modal-popup";ChangePasswordPanel.noURL=true;var


RegisterPanel=function(_PSRoomPanel11){function RegisterPanel(){var _this15;for(var _len9=arguments.length,args=new Array(_len9),_key9=0;_key9<_len9;_key9++){args[_key9]=arguments[_key9];}_this15=_PSRoomPanel11.call.apply(_PSRoomPanel11,[this].concat(args))||this;_this15.
...
handleRegisterUser=function(ev){var _this15$base,_this15$base2,_this15$base3;
ev.preventDefault();
var captcha=(_this15$base=_this15.base)==null||(_this15$base=_this15$base.querySelector('input[name=captcha]'))==null?void 0:_this15$base.value;
var password=(_this15$base2=_this15.base)==null||(_this15$base2=_this15$base2.querySelector('input[name=password]'))==null?void 0:_this15$base2.value;
var cpassword=(_this15$base3=_this15.base)==null||(_this15$base3=_this15$base3.querySelector('input[name=cpassword]'))==null?void 0:_this15$base3.value;
if(!(captcha!=null&&captcha.length)||
!(password!=null&&password.length)||
!(cpassword!=null&&cpassword.length))return _this15.setState({errorMsg:"All fields are required"});
if(password!==cpassword)return _this15.setState({errorMsg:'Passwords do not match'});
...
if(data!=null&&data.actionerror)_this15.setState({errorMsg:data==null?void 0:data.actionerror});
...
console.error(err);
_this15.setState({errorMsg:err.message});
});

_this15.setState({errorMsg:''});
};return _this15;}_inheritsLoose(RegisterPanel,_PSRoomPanel11);var _proto14=RegisterPanel.prototype;_proto14.
...
};return RegisterPanel;}(PSRoomPanel);RegisterPanel.id="register";RegisterPanel.routes=["register"];RegisterPanel.location="modal-popup";RegisterPanel.noURL=true;RegisterPanel.rightPopup=true;var


BackgroundListPanel=function(_PSRoomPanel12){function BackgroundListPanel(){var _this16;for(var _len10=arguments.length,args=new Array(_len10),_key10=0;_key10<_len10;_key10++){args[_key10]=arguments[_key10];}_this16=_PSRoomPanel12.call.apply(_PSRoomPanel12,[this].concat(args))||this;_this16.
...
if(bg==='custom'){var _this16$props$room$ar;
PSBackground.set(((_this16$props$room$ar=_this16.props.room.args)==null?void 0:_this16$props$room$ar.bgUrl)||'','custom');
_this16.close();
}else{
PSBackground.set('',bg);
}
...
};return BackgroundListPanel;}(PSRoomPanel);BackgroundListPanel.id='changebackground';BackgroundListPanel.routes=['changebackground'];BackgroundListPanel.location='modal-popup';BackgroundListPanel.noURL=true;var


ChatFormattingPanel=function(_PSRoomPanel13){function ChatFormattingPanel(){var _this17;for(var _len11=arguments.length,args=new Array(_len11),_key11=0;_key11<_len11;_key11++){args[_key11]=arguments[_key11];}_this17=_PSRoomPanel13.call.apply(_PSRoomPanel13,[this].concat(args))||this;_this17.
...
ev.preventDefault();
ev.stopImmediatePropagation();
};return _this17;}_inheritsLoose(ChatFormattingPanel,_PSRoomPanel13);var _proto16=ChatFormattingPanel.prototype;_proto16.
...
};return ChatFormattingPanel;}(PSRoomPanel);ChatFormattingPanel.id='chatformatting';ChatFormattingPanel.routes=['chatformatting'];ChatFormattingPanel.location='modal-popup';ChatFormattingPanel.noURL=true;var


LeaveRoomPanel=function(_PSRoomPanel14){function LeaveRoomPanel(){return _PSRoomPanel14.apply(this,arguments)||this;}_inheritsLoose(LeaveRoomPanel,_PSRoomPanel14);var _proto17=LeaveRoomPanel.prototype;_proto17.
...
};return LeaveRoomPanel;}(PSRoomPanel);LeaveRoomPanel.id='confirmleaveroom';LeaveRoomPanel.routes=['confirmleaveroom'];LeaveRoomPanel.location='modal-popup';LeaveRoomPanel.noURL=true;var

BattleOptionsPanel=function(_PSRoomPanel15){function BattleOptionsPanel(){var _this18;for(var _len12=arguments.length,args=new Array(_len12),_key12=0;_key12<_len12;_key12++){args[_key12]=arguments[_key12];}_this18=_PSRoomPanel15.call.apply(_PSRoomPanel15,[this].concat(args))||this;_this18.
...
var room=_this18.getBattleRoom();
if(!room)return _this18.close();
...
_this18.handleIgnoreOpponent(value);
break;
}
case'ignorespects':{
PS.prefs.set('ignorespects',value);
_this18.handleIgnoreSpectators(value);
break;
}
case'ignorenicks':{
PS.prefs.set('ignorenicks',value);
_this18.handleIgnoreNicks(value);
break;
}
case'rightpanel':{
...
handleBattleLayout=function(ev){
var value=ev.currentTarget.value;
PS.prefs.set('battlelayout',value||null);
PS.update();
};return _this18;}_inheritsLoose(BattleOptionsPanel,_PSRoomPanel15);var _proto18=BattleOptionsPanel.prototype;_proto18.
...
var isPlaying=!!(battleRoom!=null&&battleRoom.isPlaying());
var canOfferTie=battleRoom&&(battleRoom.battle.turn>=100&&isPlaying||PS.user.group==='~');
var automaticLayout=null;
if(battleRoom){
var _PS$chooseBattleLayou=PS.chooseBattleLayout(battleRoom.width,battleRoom.height),layout=_PS$chooseBattleLayou.layout,overlayControls=_PS$chooseBattleLayou.overlayControls;
automaticLayout=""+layout+(overlayControls?'-overlay':'');
}
...
preact.h("p",{"class":"buttonbar"},
preact.h("button",{"data-cmd":"/closeand /inopener /forfeit","class":"button",disabled:!isPlaying},"Forfeit"

)," ",
preact.h("button",{"data-cmd":"/closeand /inopener /offertie","class":"button",disabled:!canOfferTie},"Offer tie ",
!canOfferTie&&"(turn 100+)"
)
),
preact.h("p",null,
preact.h("label",{"class":"checkbox"},
...
preact.h("option",{value:""},"Automatic",
automaticLayout?" ("+BATTLE_LAYOUT_LABELS[automaticLayout]+")":''
),
preact.h("option",{value:"side-by-side"},
BATTLE_LAYOUT_LABELS['side-by-side']," (DESKTOP)"
),
preact.h("option",{value:"side-by-side-overlay"},
BATTLE_LAYOUT_LABELS['side-by-side-overlay']
),
preact.h("option",{value:"top-and-bottom"},
BATTLE_LAYOUT_LABELS['top-and-bottom']," (MOBILE VERTICAL)"
),
preact.h("option",{value:"top-and-bottom-overlay"},
BATTLE_LAYOUT_LABELS['top-and-bottom-overlay']
),
preact.h("option",{value:"scrolling"},
BATTLE_LAYOUT_LABELS['scrolling']
),
preact.h("option",{value:"scrolling-overlay"},
...
!PS.prefs.onepanel&&window.innerWidth>=800&&preact.h("p",null,
preact.h("label",{"class":"checkbox"},
preact.h("input",{
...
};return BattleOptionsPanel;}(PSRoomPanel);BattleOptionsPanel.id='battleoptions';BattleOptionsPanel.routes=['battleoptions'];BattleOptionsPanel.location='modal-popup';BattleOptionsPanel.noURL=true;var


PopupRoom=function(_PSRoom2){function PopupRoom(){var _this19$args;var _this19;for(var _len13=arguments.length,args=new Array(_len13),_key13=0;_key13<_len13;_key13++){args[_key13]=arguments[_key13];}_this19=_PSRoom2.call.apply(_PSRoom2,[this].concat(args))||this;_this19.
returnValue=(_this19$args=_this19.args)==null?void 0:_this19$args.cancelValue;return _this19;}_inheritsLoose(PopupRoom,_PSRoom2);var _proto19=PopupRoom.prototype;_proto19.
...
PopupPanel=function(_PSRoomPanel16){function PopupPanel(){var _this20;for(var _len14=arguments.length,args=new Array(_len14),_key14=0;_key14<_len14;_key14++){args[_key14]=arguments[_key14];}_this20=_PSRoomPanel16.call.apply(_PSRoomPanel16,[this].concat(args))||this;_this20.
...
ev.preventDefault();
ev.stopImmediatePropagation();
var room=_this20.props.room;
room.returnValue=(_room$args3=room.args)==null?void 0:_room$args3.okValue;
var textbox=_this20.base.querySelector('input[name=value]');
if(textbox){
room.returnValue=textbox.value;
}
_this20.close();
};return _this20;}_inheritsLoose(PopupPanel,_PSRoomPanel16);var _proto20=PopupPanel.prototype;_proto20.
...
render=function render(){var _room$args4,_room$args5,_room$args6,_room$args7,_room$args8,_room$args9,_room$args10,_room$args11;
var room=this.props.room;
...
var inputMode=type==='numeric'?'numeric':undefined;
...
!!type&&preact.h("p",null,preact.h("label",{"class":"label"},((_room$args11=room.args)==null?void 0:_room$args11.label)||''," ",preact.h("input",{
...
};return PopupPanel;}(PSRoomPanel);PopupPanel.id='popup';PopupPanel.routes=['popup-*'];PopupPanel.location='modal-popup';PopupPanel.noURL=true;PopupPanel.Model=PopupRoom;var


RoomTabListPanel=function(_PSRoomPanel17){function RoomTabListPanel(){var _this21;for(var _len15=arguments.length,args=new Array(_len15),_key15=0;_key15<_len15;_key15++){args[_key15]=arguments[_key15];}_this21=_PSRoomPanel17.call.apply(_PSRoomPanel17,[this].concat(args))||this;_this21.





startingLayout=PS.prefs.onepanel;_this21.
handleLayoutChange=function(ev){
var checkbox=ev.currentTarget;
PS.prefs.onepanel=checkbox.checked?'vertical':_this21.startingLayout;
PS.update();
};return _this21;}_inheritsLoose(RoomTabListPanel,_PSRoomPanel17);var _proto21=RoomTabListPanel.prototype;_proto21.
render=function render(){
...
};return RoomTabListPanel;}(PSRoomPanel);RoomTabListPanel.id='roomtablist';RoomTabListPanel.routes=['roomtablist'];RoomTabListPanel.location='modal-popup';RoomTabListPanel.noURL=true;var

BattleTimerPanel=function(_PSRoomPanel18){function BattleTimerPanel(){return _PSRoomPanel18.apply(this,arguments)||this;}_inheritsLoose(BattleTimerPanel,_PSRoomPanel18);var _proto22=BattleTimerPanel.prototype;_proto22.
...
};return BattleTimerPanel;}(PSRoomPanel);BattleTimerPanel.id='battletimer';BattleTimerPanel.routes=['battletimer'];BattleTimerPanel.location='popup';BattleTimerPanel.noURL=true;var


RulesPanel=function(_PSRoomPanel19){function RulesPanel(){return _PSRoomPanel19.apply(this,arguments)||this;}_inheritsLoose(RulesPanel,_PSRoomPanel19);var _proto23=RulesPanel.prototype;_proto23.
...
componentDidMount=function componentDidMount(){var _room$args12,_this22=this;
_PSRoomPanel19.prototype.componentDidMount.call(this);
var room=this.props.room;
var isWarn=((_room$args12=room.args)==null?void 0:_room$args12.type)==='warn';
if(isWarn){
room.closable=false;
var timerRef=setInterval(function(){
var timeLeft=_this22.state.timeLeft;
if(timeLeft===1){
clearInterval(_this22.state.timerRef);
_this22.setState({canClose:true,timeLeft:0,timerRef:null});
}else{
_this22.setState({canClose:false,timeLeft:timeLeft-1});
}
},1000);
this.setState({timeLeft:WARNING_SECONDS,canClose:false,timerRef:timerRef});
}
};_proto23.
...
render=function render(){var _room$args13,_room$args14,_room$args15;
var room=this.props.room;
var type=(_room$args13=room.args)==null?void 0:_room$args13.type;
var isWarn=type==='warn';
var message=((_room$args14=room.args)==null?void 0:_room$args14.message)||'';
return preact.h(PSPanelWrapper,{room:room,width:((_room$args15=room.args)==null?void 0:_room$args15.width)||780},
