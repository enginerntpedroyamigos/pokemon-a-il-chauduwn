...
exportPackedTeam=function exportPackedTeam(team){
var sets=Teams.unpack(team.packedTeam);
var dex=Dex.forFormat(team.format);
return Teams["export"](sets,dex);
};PSTeambuilder.
exportTeamBackup=function exportTeamBackup(teams){var readable=arguments.length>1&&arguments[1]!==undefined?arguments[1]:false;
if(!readable)return PS.teams.packAll(teams);
var buf='';for(var _i2=0;_i2<
teams.length;_i2++){var team=teams[_i2];
var format=team.format?"["+team.format+(team.isBox?'-box':'')+"] ":'';
var folder=team.folder?team.folder+"/":'';
...
while(lines.length&&!lines[lines.length-1])lines.pop();for(var _i4=0;_i4<

lines.length;_i4++){var line=lines[_i4];
line=line.trim();
if(line===''||line==='---'){
curSet=null;
...
curTeam.format=curTeam.format.slice(0,-4);
curTeam.isBox=true;
}
if(curTeam.format.startsWith('['))curTeam.format=curTeam.format.slice(1);
if(!curTeam.format.startsWith('gen'))curTeam.format="gen6"+curTeam.format;

line=line.trim();var _this$splitPrefix=
...
};return _this;}_inheritsLoose(TeamDropdownPanel,_PSRoomPanel);var _proto=TeamDropdownPanel.prototype;_proto.getTeams=function getTeams(){var _this2=this;return PS.teams.list.filter(function(team){if(team.isBox)return false;if(_this2.gen&&!team.format.startsWith(_this2.gen))return false;if(_this2.format&&team.format!==_this2.format)return false;return true;});};_proto.
render=function render(){var _this3=this;
var room=this.props.room;
if(!room.parentElem){
return preact.h(PSPanelWrapper,{room:room},
preact.h("div",{"class":"pad"},
preact.h("p",null,"This team selector is no longer available (the challenge was cancelled or something)."),
preact.h("p",{"class":"buttonbar"},
preact.h("button",{type:"button","data-cmd":"/close","class":"button"},"Close"
...
var teamBuckets={};for(var _i6=0,_teams2=
teams;_i6<_teams2.length;_i6++){var team=_teams2[_i6];
...
var genList=[];for(var _i8=0,_PS$teams$list2=
PS.teams.list;_i8<_PS$teams$list2.length;_i8++){var _team=_PS$teams$list2[_i8];
if(_team.isBox)continue;
...
};return TeamDropdownPanel;}(PSRoomPanel);TeamDropdownPanel.id='teamdropdown';TeamDropdownPanel.routes=['teamdropdown'];TeamDropdownPanel.location='modal-popup';TeamDropdownPanel.noURL=true;var
...
return preact.h(PSPanelWrapper,{room:room},
preact.h("div",{"class":"pad"},
preact.h("p",null,"This format selector is no longer available."),
preact.h("p",{"class":"buttonbar"},
preact.h("button",{type:"button","data-cmd":"/close","class":"button"},"Close"
...
var searchID=toID(this.search);for(var _i10=0;_i10<
formats.length;_i10++){var format=formats[_i10];
...
};return FormatDropdownPanel;}(PSRoomPanel);FormatDropdownPanel.id='formatdropdown';FormatDropdownPanel.routes=['formatdropdown'];FormatDropdownPanel.location='modal-popup';FormatDropdownPanel.noURL=true;
